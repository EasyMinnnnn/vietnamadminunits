...
# ---------- CSS (your snippet + variables & sidebar) ----------
CSS = """
...
.stApp{ background: radial-gradient(1100px 560px at 12% -10%, #0C5C58 0%, #0A4D4A 65%); color:#F3FBFA; }
.block-container{ max-width:1140px; padding-top:.75rem; }

/* Sidebar */
[data-testid="stSidebar"] > div:first-child{ background:var(--emerald-soft); }
section[data-testid="stSidebar"] h2, section[data-testid="stSidebar"] h3{ color:var(--gold); }

/* Hero compact */
.hero{ position:relative; padding:20px 24px; border-radius:var(--r-xl); background: linear-gradient(180deg, #107A73 0%, var(--emerald-soft) 100%); border:1px solid var(--panel-bd); box-shadow:var(--shadow); margin:8px 0 18px; }
.hero:before{ content:""; position:absolute; left:18px; right:18px; top:8px; height:7px; background: linear-gradient(90deg, var(--gold), var(--gold-hi)); border-radius:10px; }
.hero h1{ margin:.4rem 0 .25rem; font-weight:900; letter-spacing:.2px; color:var(--gold); }
.hero p{ margin:0; color:#CFE7E5; }

/* Section card */
.card{ background:var(--panel); border:1px solid var(--panel-bd); border-radius:var(--r-xl); box-shadow:var(--shadow); padding:14px 16px; margin-bottom:14px; backdrop-filter:blur(6px); }
...
.stTextInput input, .stSelectbox div[data-baseweb="select"]>div, .stTextArea textarea, .stNumberInput input{
  background:#fff !important; color:#000 !important; height:44px; border-radius:12px !important; border:1px solid #E5E7EB !important;
}
...
.stDataFrame{ border:1.5px solid color-mix(in srgb, var(--gold) 60%, transparent); border-radius:12px; overflow:hidden; }
...
def render_map(df: pd.DataFrame):
  if {"latitude","longitude"}.issubset(df.columns) and df["latitude"].notna().any():
    ...
    layer = pdk.Layer("ScatterplotLayer", data=df.rename(columns={"latitude":"lat","longitude":"lon"}),
  ...
  if run_batch and address_col:
    try:
      with st.spinner("Đang chuẩn hóa..."):
        df_out = convert_address_column(df_preview.copy(), address=address_col, convert_mode="CONVERT_2025",
                                        inplace=False, prefix="converted_", suffix="", short_name=short_name, show_progress=True)
      st.success("✅ Xong!")
      st.dataframe(df_out.head(50), use_container_width=True)
      st.download_button("⬇️ Tải kết quả (CSV)", df_out.to_csv(index=False).encode("utf-8"),

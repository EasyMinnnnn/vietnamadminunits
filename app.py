...
# ---------- CSS: Modern emerald–gold ----------
CSS = """
<link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;600;700;800;900&display=swap" rel="stylesheet">
<style>
:root{
  --bg-1:#0C5C58; --bg-2:#0A4D4A;
  --emerald:#066E68; --emerald-soft:#0E6963;
  --gold:#D4AF37; --gold-hi:#FFD700;
  --panel:rgba(255,255,255,.045); --panel-bd:rgba(255,255,255,.08);
  --shadow:0 10px 28px rgba(0,0,0,.26);
  --r:14px; --r-xl:18px;
}

html, body, [class*="css"]{ font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,sans-serif; }
.stApp{ background: radial-gradient(1100px 560px at 12% -10%, var(--bg-1) 0%, var(--bg-2) 65%); color:#F3FBFA; }
...
/* Hero compact */
.hero{
  position:relative; padding:20px 24px; border-radius:var(--r-xl);
  background: linear-gradient(180deg, #107A73 0%, var(--emerald-soft) 100%);
  border:1px solid var(--panel-bd); box-shadow:var(--shadow); margin:8px 0 18px;
}
.hero:before{
...
/* Section card */
.card{ background:var(--panel); border:1px solid var(--panel-bd); border-radius:var(--r-xl);
...
level = st.sidebar.number_input(
    "Level", 1, 3 if mode_str == "LEGACY" else 2, 3 if mode_str == "LEGACY" else 2, step=1
)
st.sidebar.markdown("---")
...
if parse_clicked:
    try:
        st.markdown('<div class="card skel"></div>', unsafe_allow_html=True)
